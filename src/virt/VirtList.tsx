...
import type { EmitFunction, ReactiveData, SlotSize, VirtListProps, VirtListReturn } from './type';
...
function useVirtList<T extends Record<string, any>>(
  ...
  async function scrollToBottom() {
    ...
    setTimeout(() => {
      ...
      if (
        Math.abs(
          Math.round(reactiveData.offset + slotSize.clientSize) - Math.round(getTotalSize()),
        ) > 2
      ) {
        scrollToBottom();
      }
  ...
  function onScroll(evt: Event) {
    ...
    if (
      ...
      Math.round(reactiveData.offset + props.scrollDistance) >=
        Math.round(reactiveData.listTotalSize + getSlotSize() - slotSize.clientSize)
    ) {
    ...
    (newVal, oldVal) => {
      if (newVal && oldVal) {
        ...
        if (props?.renderControl) {
          const { begin, end } = props.renderControl(_newInViewBegin, reactiveData.inViewEnd);
